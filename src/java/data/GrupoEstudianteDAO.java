...
public class GrupoEstudianteDAO {
    ...
    public void updateNota(GrupoEstudiante gE) throws Exception {
        String sql="update gruposestudiantes set nota=%s "+
                "where id_estudiante=%s and id_grupo=%s";
        sql = String.format(sql, gE.getNota(), gE.getEstudiante().getId(), gE.getGrupo().getId());
        PreparedStatement stm = Connection.instance().prepareStatement(sql);
